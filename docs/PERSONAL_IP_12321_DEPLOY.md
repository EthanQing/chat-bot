...
> 建议：如果已经有域名，优先使用腾讯云/Let's Encrypt 等正式证书，再跳到第 6 步把证书路径替换为正式证书路径。自签证书只作为“没有域名时能访问”的临时方案。
...
这是自签证书的正常现象。个人测试可以选择继续访问；如果出现同步请求失败、后台恢复后数据不更新、流式连接不稳定，优先更换为正式 HTTPS 证书。
